...
class AzureWorkloadIdentityTokenProvider {
  ...
  private const DEFAULT_TOKEN_FILE = "default_workload_identity_token_file";  // Default cache filename
  private const DEFAULT_SCOPE = 'https://ossrdbms-aad.database.windows.net/.default';  // Default Azure scope
  private const EXPIRES_BEFORE_SEC = 600;  // Buffer time for token expiration (10 minutes)
  private const DEFAULT_AUTHORITY_HOST = 'https://login.microsoftonline.com/';  // Default Azure authority host

  // Instance configuration properties
  /** @var string Azure Client ID from environment or constructor */
  private string $clientId;
  /** @var string Azure Tenant ID from environment or constructor */
  private string $tenantId;
  /** @var string Path to federated token file from environment or constructor */
  private string $federatedTokenFile;
  /** @var string Azure authority host URL */
  private string $authorityHost;

  /**
   * Initialize token provider with configuration
   * 
   * @param string|null $clientId           Azure Client ID (default: AZURE_CLIENT_ID env)
   * @param string|null $tenantId           Azure Tenant ID (default: AZURE_TENANT_ID env)
   * @param string|null $federatedTokenFile  Path to federated token file (default: AZURE_FEDERATED_TOKEN_FILE env)
   * @param string|null $authorityHost      Azure authority host URL (default: AZURE_AUTHORITY_HOST env or DEFAULT_AUTHORITY_HOST)
   */
   ...
   * 
   * @param string $scope               Azure scope for the token (default: DEFAULT_SCOPE)
   * @param string|null $cachedTokenFileName  Custom cache filename (default: DEFAULT_TOKEN_FILE)
   * @return string                     Valid access token
   * @throws RuntimeException          On configuration or token retrieval errors
   */
   ...
   * 
   * @param string $cachedTokenFileName Cache filename to check
   * @return string|null Valid access token or null if not available/expired
   */
   ...
   * 
   * @param string $scope               Azure scope for the token
   * @param string $cachedTokenFileName Cache filename to use
   * @return string                     New access token
   * @throws RuntimeException          On token retrieval or cache failures
   */
   ...
   * 
   * @param array $credentials Request parameters
   * @return array Decoded token response
   * @throws RuntimeException On request failures or invalid responses
   */
